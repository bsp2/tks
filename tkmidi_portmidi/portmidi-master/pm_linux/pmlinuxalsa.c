...
static PmError check_hosterror(int err, int _line)
{
    if (err < 0) {
        printf("[---] portmidi<pmlinuxalsa.c>: host error=%d line=%d\n", err, _line);
...
static PmError alsa_use_queue(void)
{
    ...
    if (queue_used == 0) {
        ...
        if (queue < 0) {
            return check_hosterror(queue, __LINE__);
        }
        ...
        if (err < 0) {
            return check_hosterror(err, __LINE__);
        }

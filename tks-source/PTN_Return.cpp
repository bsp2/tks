...
void PTN_Return::eval(PTN_Env *_env) const {
   ...
   {
      _env->continue_flag = YAC_FALSE; // same as PTN_Return__eval(_env, this), but w/o fxn call
   }
}
...
sBool PTN_Return::forceHybrid(void) {
   return YAC_FALSE; // must use interpreted exec because PTN_Env is not available in JIT
}

...
sBool YAC_VCALL YAC_HashTable::yacToString(YAC_String *_s) const {
   return yacToString2(_s, YAC_FALSE/*bReparsable*/);
}
...
sBool YAC_HashTable::yacToString2(YAC_String *_s, sBool _bReparsable) const {
   ...
   _s->alloc(2u + varcache.num_entries * 32u + 1u);
   _s->empty();
   _s->append("#[");
   sUI i;
   YAC_String s;
            ...
            switch(c->type)
            {
               ...
               case YAC_TYPE_STRING:
                  ...
                  else
                  {
                     if(c->isNull())
                     {
                        _s->append(_bReparsable ? "null" : "<null>");
                     }
                     else
                     {
                        _s->append("*ILL*");
...
void YAC_HashTable::_getString(YAC_Value *_r) const {
   _r->initEmptyString();
   yacToString2(_r->value.string_val, YAC_FALSE/*bReparsable*/);
}

void YAC_HashTable::_getReparsableString(YAC_Value *_r) const {
   _r->initEmptyString();
   yacToString2(_r->value.string_val, YAC_TRUE/*bReparsable*/);
}
...
sUI YAC_VCALL YAC_HashTable::yacDeserialize(YAC_Object *_ifs, sUI _rtti) {
   ...
   // Dyac_host_printf("xxx YAC_HashTable::yacDeserialize: me=%u ne=%u\n", me, ne);
   ...
   if(YAC_OP_ASSIGN == _cmd)
   {
      if(_o != ((YAC_Object*)this))
      {
         if(YAC_CHK(_o, YAC_CLID_HASHTABLE))
         {
            ...
            varcache.freeCacheEntries();

            if(o->varcache.max_entries >= varcache.max_entries)
            {
               varcache.allocCache(o->varcache.max_entries);
            }
...
void YAC_HashTable::addEntriesFrom(YAC_HashTable *_o) {
   ...
   for(sUI i = 0u; i < _o->varcache.max_entries; i++)
   {
...
sSI YAC_HashTable::_addInt(YAC_String *_s, sSI _val) {
   TKS_CachedObject *co = varcache.createIntegerEntry(_s, YAC_TRUE/*bCopyName*/);
   if(co)
   {
...
sSI YAC_HashTable::_addFloat(YAC_String *_s, sF32 _val) {
   TKS_CachedObject *co=varcache.createFloatEntry(_s, YAC_TRUE/*bCopyName*/);
   if(co)
   {
...
sSI YAC_HashTable::_addString(YAC_String *_s, YAC_String *_v) {
   TKS_CachedObject *co = varcache.createStringInstance(_s, YAC_TRUE/*bCopyName*/);
   if(co)
	{
...
sSI YAC_HashTable::_addObject(YAC_String *_s, YAC_Object *_v) {
   ...
   if(_v)
   {
      ...
      co = varcache.createObjectEntry(_s, YAC_TRUE/*bCopyName*/, no, YAC_TRUE/*del*/);
   }
   return (NULL != co);
}
...
sSI YAC_HashTable::_addObjectRef(YAC_String *_s, YAC_Object *_v) {
   return
      _v
      ? (varcache.createObjectEntry((YAC_String*)_s,
                                    YAC_TRUE/*bCopyName*/,
                                    _v,
                                    YAC_FALSE/*del*/
                                    ) != 0
...
void YAC_VCALL YAC_HashTable::yacHashSet(void *_context, YAC_String *_key, YAC_Value *_value) {
   ...
   TKS_CachedObject *co = varcache.createEntry(_key, YAC_TRUE/*bCopyName*/);
   if(co)
   {
...
void YAC_HashTable::_setObject(YAC_Object *_key, YAC_Object *_value) {
   if(YAC_Is_String(_key))
   {
      YAC_Value v; v.initObject(_value, YAC_FALSE/* reference only */ );
      yacHashSet(NULL, (YAC_String*)_key, &v); // xxx TKS_MT: should use *real* thread context (exceptions)
   }
   else
   {
...
void YAC_HashTable::_setString(YAC_Object *_key, YAC_Object *_s) {
   if(YAC_Is_String(_key))
   {
      if(YAC_BCHK(_s, YAC_CLID_STRING))
      {
         ...
         s->yacCopy(_s);
         YAC_Value v; v.initString(s, YAC_TRUE/*del*/);
         yacHashSet(NULL, (YAC_String*)_key, &v); // xxx TKS_MT: should use *real* thread context (exceptions)
      }
      else
      {
   ...
   if(YAC_VALID(_array))
   {
      ...
      if(_array->yacArrayAlloc(l, 0,0,0))
      {
         ...
         while(i < varcache.max_entries)
         {
            ...
            if(co)
            {
               YAC_Value v; v.initString(&co->name, YAC_FALSE/*del*/);

...
bool SidTune::loadYACStream(YAC_Object *_ifs, const bool separatorIsSlash)
{
   cleanup();
   init();
   isSlashedFileName = separatorIsSlash;
   getFromYACStream(_ifs);
   return status;
}
...
bool SidTune::loadYACStream(YAC_Object *_ifs, Buffer_sidtt<const uint_least8_t>& bufferRef)
{
   Buffer_sidtt<uint_least8_t> fileBuf;
   uint_least32_t fileLen = 0;
   
	fileLen= _ifs->yacStreamGetSize();
   ...
   if ( !fileBuf.assign(new uint_least8_t[fileLen],fileLen) )
   {
	...
	//myIn.seekg(0,std::ios::beg);
	uint_least32_t restFileLen = fileLen;
	_ifs->yacStreamRead( (sU8*)fileBuf.get(), fileLen);
	if ( _ifs->yacStreamGetErrorCode() )
	{
		info.statusString = SidTune::txt_cantLoadFile;
		return false;
	}
...
void SidTune::getFromYACStream(YAC_Object *_ifs)
{
   // Assume a failure, so we can simply return.
   status = false;

...
struct StDelay {
   ...
   float readNearest(unsigned int _offset) {
      _offset = (io_offset - 1 - _offset) & ST_DELAY_MASK;
      last_out = history[_offset];
      return last_out;
   }

   float readNearestNorm(float _offsetNorm) {
      int offset =  (int)(_offsetNorm * (float)(ST_DELAY_SIZE - 1u));
      if(offset < 0)
         offset = 0;
      else if(offset > (ST_DELAY_SIZE - 1))
         offset = (ST_DELAY_SIZE - 1);
      return readNearest(offset);
   }

   float readLinear(float _offset) {
      unsigned int offC = (unsigned int)(_offset + 1u);
      unsigned int offN = (unsigned int)(_offset + 2u);

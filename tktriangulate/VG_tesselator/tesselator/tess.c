...
{
  Tesselator *tess;
  tess = (Tesselator *)malloc(sizeof(Tesselator));
...
// Turns the processed tesselator structure into a single VGES Element.
{
  ...
  out = (Element *) malloc (size1 + size2 + size3);
  if (!out) return 0;
  ...
  for( f = tess->mesh->fHead.next; f != &tess->mesh->fHead; f = f->next ) 
  {
    ...
    if (f->inside) 
    {
      ...
      if (!out)
      {
        ...
        // allocate all parts in one chunk:
        out = (Element *) malloc (size1 + size2);
        if (!out) return 0;
...
Element * VGES_TesselatateElements (VGTesselator a_tess, Element * src, VGFillRule WindingRule, VGboolean CreateEdgeFlags)
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
{
  Element    * out  = 0;
  Tesselator * tess = (Tesselator *) a_tess;

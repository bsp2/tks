...
sU8 *yac_string_alloc_chars(size_t _n) {
   yac_string_total_char_size += sSI(_n);
...
void yac_string_free_chars(sU8 *_chars) {
   // (note) must _NEVER_ free chars obtained from actual host
...
YAC_ListNode::~YAC_ListNode                             ()                                                 {if(next){YAC_DELETE(next);next=0;}unset();}
#endif
...
YAC_TreeNode::~YAC_TreeNode                             ()                                                 {if(left){YAC_DELETE(left);left=0;}if(right){YAC_DELETE(right); right=0;}unset();}
#endif
...
sBool            YAC_VCALL YAC_Object::yacArrayAlloc                  (sUI, sUI, sUI, sUI)                    {return 0;}
...
sF32             YAC_VCALL YAC_Object::yacEnvGetValue                 (void)                                  {return 0.0f;}
void             YAC_VCALL YAC_Object::yacEnvTickPrecise              (sF32 _dt)                              {(void)_dt;}
void             YAC_VCALL YAC_Object::yacEnvSetTime                  (sF32 _t)                               {(void)_t;}
...
#endif // YAC_OBJECT_POOL
#if defined(YAC_OBJECT_POOL) && !defined(YAC_NO_HOST)
...
sBool YAC_Object::_yacArrayAlloc(sSI _sx, sSI _sy, sSI _type, sSI _ebytesize) {
   return yac_object_yacArrayAlloc(this, _sx, _sy, _type, _ebytesize);
}
sBool YAC_Object::_yacArrayRealloc(sSI _sx, sSI _sy, sSI _type, sSI _ebytesize) {
...
void YAC_CALL yac_object_yacMemberGetTypes(YAC_Object *_o, YAC_Value *_r) {
   YAC_IntArray *a = YAC_New_IntArray();
   if(a)
   {
      sSI num = (sSI)_o->yacMemberGetNum();
      if(num && a->yacArrayAlloc(num, 0,0,0))
...
void YAC_CALL yac_object_yacMethodGetNames(YAC_Object *_o, YAC_Value *_r) {
   ...
   if(a)
   {
      sSI num = (sSI)_o->yacMethodGetNum();
      if(num && a->yacArrayAlloc(num, 0,0,0))
      {
         a->num_elements = a->max_elements;
...
void YAC_CALL yac_object_yacMethodGetParameterTypes(YAC_Object *_o, YAC_Value *_r) {
   YAC_ValueArray *a = YAC_New_ValueArray();
   if(a)
   {
      ...
      if(num&&a->yacArrayAlloc(num, 0,0,0))
      {
         ...
         for(sSI i=0; i<num; i++)
         {
            YAC_IntArray *ia = YAC_New_IntArray();
            if(ia)
            {
            ...
            {
               Dyac_host_printf("[---] yac_object_yacMethodGetParameterTypes: cannot allocate inner array %i/%i.\n", i, num);
               YAC_DELETE(a);
               YAC_RETO(0, 0);
               return;
            }
...
void YAC_CALL yac_object_yacMethodGetParameterObjectTypes(YAC_Object *_o, YAC_Value *_r) {
   ...
   if(a)
   {
      ...
      if(num && a->yacArrayAlloc(num, 0,0,0))
      {
         ...
         for(sSI i = 0; i < num; i++)
         {
            YAC_StringArray *ia = YAC_New_StringArray();
            if(ia)
            {
               a->elements[i].initObject(ia, 1);
               sSI inum = (sSI)numParameters[i];
               if(inum && ia->yacArrayAlloc(inum, 0,0,0))
               {
                  ...
                  for(sSI j = 0; j < inum; j++)
                  {
//               Dyac_host_printf("ia->elements[%i]=%p (%s) *s=%c\n", j, parameterObjectTypes[j],
...
//                                parameterObjectTypes[j]?parameterObjectTypes[j][0]:'?');
            ...
            {
               Dyac_host_printf("[---] yac_object_yacMethodGetParameterObjectTypes: cannot allocate inner array %i/%i.\n", i, num);
               YAC_DELETE(a);
               YAC_RETO(0, 0);
               return;
            }
   ...
   if(a)
   {
      ...
      if(num && a->yacArrayAlloc(num, 0,0,0))
      {
         ...
         for(sSI i = 0; i < num; i++)
         {
            ...
            {
               case YAC_TYPE_INT:
                  a->elements[i].initInt(constantValues.s32[i]);
                  break;
               case YAC_TYPE_FLOAT:
                  a->elements[i].initFloat(constantValues.f32[i]); // xxx 32bit float assumed
                  break;
               case YAC_TYPE_OBJECT:
               default:
                  a->elements[i].initObject(0, 0);
                  Dyac_host_printf("[---] yac_object_yacConstantGetValues(): constant %i has YAC_TYPE_OBJECT(%i) (not supported).\n", i, constantTypes[i]);
                  break;
            }
         }
      }
...
sBool YAC_CALL yac_object_yacArrayAlloc(YAC_Object *_o, sSI _sx, sSI _sy, sSI _type, sSI _ebytesize) {
   return _o->yacArrayAlloc((sUI)_sx, (sUI)_sy, (sUI)_type, (sUI)_ebytesize);
}

sBool YAC_CALL yac_object_yacArrayRealloc(YAC_Object *_o, sSI _sx, sSI _sy, sSI _type, sSI _ebytesize) {
   return _o->yacArrayRealloc((sUI)_sx, (sUI)_sy, (sUI)_type, (sUI)_ebytesize);
}
...
sBool            YAC_String::append                   (YAC_String *o)             {
...
void             YAC_Host::printf                     (const char *_fmt, ...)      {
   if(_fmt)
   {
      ...
      va_start(va,_fmt);
#ifdef YAC_MACOS
      ::vsnprintf(buf, sizeof(buf)/sizeof(char), _fmt, va);
#else
...
sSI YAC_CALL yac_fltcmp_rel_fast(sF32 a, sF32 b) {
   ...
   if( (a == 0.0f) || (b == 0.0f) )
   {
      _c.f32 = yac_epsilon_flt;
   }
   else
   {
      _c.s32 = yac_epsilon_flt_units;
   }
   if(_a.s32 < 0)
   {
      _a.u32 = 0x80000000 - _a.u32;
   }
   if(_b.s32 < 0)
   {
      _b.u32 = 0x80000000 - _b.u32;
   }
   if(abs(_a.s32 - _b.s32) <= _c.s32)
   {
      return 0;
   }
   else if(a < b)
...
sSI YAC_CALL yac_dblcmp_rel_fast(sF64 a, sF64 b) {
   ...
   if( (a == 0.0) || (b == 0.0) )
   {
      _c.f64 = yac_epsilon_dbl;
   }
   else
   {
      _c.s64 = yac_epsilon_dbl_units;
   }
   if(_a.s64 < 0)
   {
      _a.u64 = 0x8000000000000000ull - _a.u64;
   }
   if(_b.s64 < 0)
   {
      _b.u64 = 0x8000000000000000ull - _b.u64;
   }
   res = _a.s64 - _b.s64;
   if(res < 0)
   {

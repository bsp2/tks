...
public:
	...
	void frequency(float freq) {
		...
		phase_increment = uint32_t( freq * (4294967296.0f / AUDIO_SAMPLE_RATE_EXACT) );
		if (phase_increment > 0x7FFE0000u)
			phase_increment = 0x7FFE0000u;
	}
	void phase(float angle) {
		...
		phase_offset = uint32_t( angle * (float)(4294967296.0 / 360.0) );
	}
	...
	void offset(float n) {
		...
		tone_offset = int16_t( n * 32767.0f );
	}
	void pulseWidth(float n) {	// 0.0 to 1.0
		...
		pulse_width = uint32_t( n * 4294967296.0f );
	}
...
public:
	...
	void frequency(float freq) {
		...
		phase_increment = uint32_t( freq * (4294967296.0f / teensy_sample_rate) );
		if (phase_increment > 0x7FFE0000u)
			phase_increment = 0x7FFE0000u;
	}
	void amplitude(float n) {	// 0 to 1.0
	...
	void frequencyModulation(float octaves) {
		...
		modulation_factor = uint32_t( octaves * 4096.0f );
		modulation_type = 0;
	}
	void phaseModulation(float degrees) {
		...
		modulation_factor = uint32_t( degrees * (float)(65536.0 / 180.0) );
		modulation_type = 1;
	}

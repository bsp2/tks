...
extern float teensy_sample_rate;
...
typedef RingBuffer<int16_t, AUDIO_BLOCK_SAMPLES> TeensyBuffer;
...
public:
	...
	// uniform on [-1, 1]
	float process() {
		return float(level_ * ((rand31pm_next() / 2147483647.0) * 2.f - 1.f));
	}

	// uniform on [0, 1]
	float processNonnegative() {
		return float(level_ * (rand31pm_next() / 2147483647.0));
	}

...
typedef struct resample_nearest_info_s {
   st_plugin_info_t base;
} resample_nearest_info_t;

typedef struct resample_nearest_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} resample_nearest_shared_t;

typedef struct resample_nearest_voice_s {
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(resample_nearest_shared_t);
...
static void ST_PLUGIN_API loc_note_on(st_plugin_voice_t  *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(resample_nearest_voice_t);
   (void)_bGlide;
   (void)_note;
   (void)_vel;
   if(!_bGlide)
...
static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(resample_nearest_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(resample_nearest_shared_t);
   (void)_freqHz;
   (void)_note;
   (void)_vol;
   (void)_pan;
...
static void ST_PLUGIN_API loc_process_replace(st_plugin_voice_t  *_voice,
   ...
   if(_bMonoIn)
   {
      ...
      for(unsigned int i = 0u; i < _numFrames; i++)
      {
         ...
         if(voice->phase > lastPhase)
            out = voice->last_sample_l;
         else
            out = voice->last_sample_l = l;
   ...
   else
   {
      ...
      for(unsigned int i = 0u; i < _numFrames; i++)
      {
         ...
         if(voice->phase > lastPhase)
         {
            outL = voice->last_sample_l;
            outR = voice->last_sample_r;
         }
         else
         {
            outL = voice->last_sample_l = l;
            outR = voice->last_sample_r = r;
         }
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   resample_nearest_shared_t *ret = malloc(sizeof(resample_nearest_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   resample_nearest_voice_t *ret = malloc(sizeof(resample_nearest_voice_t));
   if(NULL != ret)
   {
      memset(ret, 0, sizeof(*ret));
      ret->base.info   = _info;
...
st_plugin_info_t *resample_nearest_init(void) {
   resample_nearest_info_t *ret = NULL;

   ret = malloc(sizeof(resample_nearest_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp resample nearest";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "resample nearest";
      ret->base.short_name  = "res near";

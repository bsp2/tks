...
typedef struct x_min_info_s {
   st_plugin_info_t base;
} x_min_info_t;

typedef struct x_min_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} x_min_shared_t;

typedef struct x_min_voice_s {
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(x_min_shared_t);
...
static void ST_PLUGIN_API loc_note_on(st_plugin_voice_t  *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(x_min_voice_t);
   (void)_bGlide;
   (void)_note;
   (void)_vel;
   if(!_bGlide)
...
static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(x_min_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(x_min_shared_t);
   (void)_freqHz;
   (void)_note;
   (void)_vol;
   (void)_pan;
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   x_min_shared_t *ret = malloc(sizeof(x_min_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   x_min_voice_t *ret = malloc(sizeof(x_min_voice_t));
   if(NULL != ret)
   {
      memset(ret, 0, sizeof(*ret));
      ret->base.info   = _info;
...
st_plugin_info_t *x_min_init(void) {
   x_min_info_t *ret = NULL;

   ret = malloc(sizeof(x_min_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp x min";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "x min";
      ret->base.short_name  = "x min";

// ----
// ---- file   : biquad_hpf_3.cpp
// ---- author : Bastian Spiegel <bs@tkscript.de>
// ---- legal  : (c) 2020-2026 by Bastian Spiegel. 
// ----          Distributed under terms of the GNU LESSER GENERAL PUBLIC LICENSE (LGPL). See 
// ----          http://www.gnu.org/licenses/licenses.html#LGPL or COPYING for further information.
// ----
// ---- info   : a biquad high pass filter that supports per-sample-frame parameter interpolation
// ----
// ---- created: 21May2020
...
typedef struct biquad_hpf_3_info_s {
   st_plugin_info_t base;
} biquad_hpf_3_info_t;

typedef struct biquad_hpf_3_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} biquad_hpf_3_shared_t;

typedef struct biquad_hpf_3_voice_s {
   ...
   float    mod_drive_cur;
   float    mod_drive_inc;
   StBiquad hpf_l[NUM_POLES];
   StBiquad hpf_r[NUM_POLES];
} biquad_hpf_3_voice_t;


static void ST_PLUGIN_API loc_set_sample_rate(st_plugin_voice_t *_voice,
                                              float              _sampleRate
                                              ) {
   ST_PLUGIN_VOICE_CAST(biquad_hpf_3_voice_t);
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(biquad_hpf_3_shared_t);
...
static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(biquad_hpf_3_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(biquad_hpf_3_shared_t);
   (void)_freqHz;
   (void)_note;
   (void)_vol;
   (void)_pan;
   ...
   for(unsigned int i = 0u; i < NUM_POLES; i++)
   {
      voice->hpf_l[i].calcParams(_numFrames,
                                 StBiquad::HPF,
                                 0.0f/*gainDB*/,
                                 modFreqL,
                                 modQ
      ...
      voice->hpf_r[i].calcParams(_numFrames,
                                 StBiquad::HPF,
                                 0.0f/*gainDB*/,
                                 modFreqR,
                                 modQ
...
static void ST_PLUGIN_API loc_process_replace(st_plugin_voice_t  *_voice,
   ...
   for(unsigned int i = 0u; i < _numFrames; i++)
   {
      ...
      for(unsigned int i = 0u; i < NUM_POLES; i++)
      {
         outL = voice->hpf_l[i].filter(outL);
         outR = voice->hpf_r[i].filter(outR);
      }
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   biquad_hpf_3_shared_t *ret = (biquad_hpf_3_shared_t *)malloc(sizeof(biquad_hpf_3_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   biquad_hpf_3_voice_t *ret = (biquad_hpf_3_voice_t *)malloc(sizeof(biquad_hpf_3_voice_t));
   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));
      ret->base.info = _info;
...
extern "C" {
st_plugin_info_t *biquad_hpf_3_init(void) {
   biquad_hpf_3_info_t *ret = NULL;

   ret = (biquad_hpf_3_info_t *)malloc(sizeof(biquad_hpf_3_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp biquad hpf 3";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "biquad high pass filter 3";
      ret->base.short_name  = "bq hpf 3";

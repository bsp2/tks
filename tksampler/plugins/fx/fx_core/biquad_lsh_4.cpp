...
typedef struct biquad_lsh_4_info_s {
   st_plugin_info_t base;
} biquad_lsh_4_info_t;

typedef struct biquad_lsh_4_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} biquad_lsh_4_shared_t;

typedef struct biquad_lsh_4_voice_s {
   ...
   float    mod_drive_cur;
   float    mod_drive_inc;
   StBiquad lsh_l[NUM_POLES];
   StBiquad lsh_r[NUM_POLES];
} biquad_lsh_4_voice_t;


static void ST_PLUGIN_API loc_set_sample_rate(st_plugin_voice_t *_voice,
                                              float              _sampleRate
                                              ) {
   ST_PLUGIN_VOICE_CAST(biquad_lsh_4_voice_t);
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(biquad_lsh_4_shared_t);
...
static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(biquad_lsh_4_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(biquad_lsh_4_shared_t);
   (void)_freqHz;
   (void)_note;
   (void)_vol;
   (void)_pan;
   ...
   for(unsigned int i = 0u; i < NUM_POLES; i++)
   {
      voice->lsh_l[i].calcParams(_numFrames,
                                 StBiquad::LSH,
                                 modGain,
                                 modFreqL,
                                 0.0f/*q*/
      ...
      voice->lsh_r[i].calcParams(_numFrames,
                                 StBiquad::LSH,
                                 modGain,
                                 modFreqR,
                                 0.0f/*q*/
...
static void ST_PLUGIN_API loc_process_replace(st_plugin_voice_t  *_voice,
   ...
   for(unsigned int i = 0u; i < _numFrames; i++)
   {
      ...
      for(unsigned int i = 0u; i < NUM_POLES; i++)
      {
         outL = voice->lsh_l[i].filter(outL);
         outR = voice->lsh_l[i].filter(outR);
      }
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   biquad_lsh_4_shared_t *ret = (biquad_lsh_4_shared_t *)malloc(sizeof(biquad_lsh_4_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   biquad_lsh_4_voice_t *ret = (biquad_lsh_4_voice_t *)malloc(sizeof(biquad_lsh_4_voice_t));
   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));
      ret->base.info = _info;
...
extern "C" {
st_plugin_info_t *biquad_lsh_4_init(void) {
   biquad_lsh_4_info_t *ret = NULL;

   ret = (biquad_lsh_4_info_t *)malloc(sizeof(biquad_lsh_4_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp biquad lsh 4";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "biquad low shelf 4";
      ret->base.short_name  = "bq lsh 4";

...
typedef struct biquad_hpf_1_info_s {
   st_plugin_info_t base;
} biquad_hpf_1_info_t;

typedef struct biquad_hpf_1_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} biquad_hpf_1_shared_t;

typedef struct biquad_hpf_1_voice_s {
   ...
   StBiquad hpf_1_l;
   StBiquad hpf_1_r;
} biquad_hpf_1_voice_t;


static void ST_PLUGIN_API loc_set_sample_rate(st_plugin_voice_t *_voice,
                                              float              _sampleRate
                                              ) {
   ST_PLUGIN_VOICE_CAST(biquad_hpf_1_voice_t);
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(biquad_hpf_1_shared_t);
...
static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(biquad_hpf_1_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(biquad_hpf_1_shared_t);
   (void)_freqHz;
   (void)_note;
   (void)_vol;
   (void)_pan;
   ...
   voice->hpf_1_l.calcParams(_numFrames,
                             StBiquad::HPF,
                             0.0f/*gainDB*/,
                             modFreqL,
                             modQ
   ...
   voice->hpf_1_r.calcParams(_numFrames,
                             StBiquad::HPF,
                             0.0f/*gainDB*/,
                             modFreqR,
                             modQ
...
static void ST_PLUGIN_API loc_process_replace(st_plugin_voice_t  *_voice,
   ...
   for(unsigned int i = 0u; i < _numFrames; i++)
   {
      ...
      float outL = voice->hpf_1_l.filter(l * voice->mod_drive_cur);
      float outR = voice->hpf_1_r.filter(r * voice->mod_drive_cur);
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   biquad_hpf_1_shared_t *ret = (biquad_hpf_1_shared_t *)malloc(sizeof(biquad_hpf_1_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   biquad_hpf_1_voice_t *ret = (biquad_hpf_1_voice_t *)malloc(sizeof(biquad_hpf_1_voice_t));
   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));
      ret->base.info = _info;
...
extern "C" {
st_plugin_info_t *biquad_hpf_1_init(void) {
   biquad_hpf_1_info_t *ret = NULL;

   ret = (biquad_hpf_1_info_t *)malloc(sizeof(biquad_hpf_1_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp biquad hpf 1";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "biquad high pass filter 1";
      ret->base.short_name  = "bq hpf 1";

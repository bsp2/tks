...
typedef struct dly_1_fade_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} dly_1_fade_shared_t;

typedef struct dly_1_fade_voice_s {
...
static void ST_PLUGIN_API loc_set_sample_rate(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(dly_1_fade_voice_t);
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(dly_1_fade_shared_t);
...
static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(dly_1_fade_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(dly_1_fade_shared_t);
   (void)_freqHz;
   (void)_note;
   (void)_vol;
   (void)_pan;
...
static void ST_PLUGIN_API loc_process_replace(st_plugin_voice_t  *_voice,
   ...
   //  (note) fade between old and new delay time to minimize clicks during delay time modulation (e.g. chorus)
   float c;
   float n;
   unsigned int k = 0u;
   for(unsigned int i = 0u; i < _numFrames; i++)
   {
      ...
      c = voice->dly_l.readLinear(voice->mod_time_cur);
      n = voice->dly_l.readLinear(voice->mod_time_next);
      float outL = c + (n - c) * voice->time_fade_amt;
      voice->dly_l.pushRaw(l + outL * voice->mod_fb_cur);

      float r = _samplesIn[k + 1u];
      c = voice->dly_r.readLinear(voice->mod_time_cur);
      n = voice->dly_r.readLinear(voice->mod_time_next);
      float outR = c + (n - c) * voice->time_fade_amt;
      voice->dly_r.pushRaw(r + outR * voice->mod_fb_cur);
      ...
      {
         voice->mod_time_cur  = voice->mod_time_next;
         voice->mod_time_next = voice->mod_time_target;
         voice->time_fade_amt = 0.0f;
      }
      else
      {
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   dly_1_fade_shared_t *ret = (dly_1_fade_shared_t *)malloc(sizeof(dly_1_fade_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   dly_1_fade_voice_t *ret = (dly_1_fade_voice_t *)malloc(sizeof(dly_1_fade_voice_t));
   if(NULL != ret)
   {
...
extern "C" {
st_plugin_info_t *dly_1_fade_init(void) {

   dly_1_fade_info_t *ret = (dly_1_fade_info_t *)malloc(sizeof(dly_1_fade_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp dly 1 fade";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "delay 1 fade";
      ret->base.short_name  = "dly 1 fd";

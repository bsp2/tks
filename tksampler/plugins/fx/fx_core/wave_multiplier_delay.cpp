// ----
// ---- file   : wave_multiplier_delay.cpp
// ---- author : Bastian Spiegel <bs@tkscript.de>
// ---- legal  : (c) 2021-2026 by Bastian Spiegel.
// ----          Distributed under terms of the GNU LESSER GENERAL PUBLIC LICENSE (LGPL). See
// ----          http://www.gnu.org/licenses/licenses.html#LGPL or COPYING for further information.
// ----
// ---- info   : pseudo phase shifter
// ----
// ---- created: 13Oct2021
// ---- changed: 21Jan2024, 01May2026, 11Sep2026
// ----
// ----
// ----

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>

#include "../../../plugin.h"

#define ST_DELAY_SIZE 1024
#define ST_DELAY_MASK 1023
#include "delay.h"

#define PARAM_DRYWET   0
#define PARAM_SMOOTH   1
#define PARAM_SHIFT1   2
#define PARAM_LEVEL1   3
#define PARAM_SHIFT2   4
#define PARAM_LEVEL2   5
#define PARAM_SHIFT3   6
#define PARAM_LEVEL3   7
#define NUM_PARAMS     8
#ifndef STFX_SKIP_NAMES_AND_RESETS
static const char *loc_param_names[NUM_PARAMS] = {
   "Dry / Wet",
   "Smooth",
   "Shift 1",
   "Level 1",
   "Shift 2",
   "Level 2",
   "Shift 3",
   "Level 3",
};
static float loc_param_resets[NUM_PARAMS] = {
   1.0f,  // DRYWET
   0.9f,  // SMOOTH
   0.0f,  // SHIFT1
   0.0f,  // LEVEL1
   0.0f,  // SHIFT2
   0.0f,  // LEVEL2
   0.0f,  // SHIFT3
   0.0f,  // LEVEL3
};
#endif // !STFX_SKIP_NAMES_AND_RESETS

#define MOD_DRYWET  0
#define MOD_SMOOTH  1
#define MOD_SHIFT1  2
#define MOD_LEVEL1  3
#define MOD_SHIFT2  4
#define MOD_LEVEL2  5
#define MOD_SHIFT3  6
#define MOD_LEVEL3  7
#define NUM_MODS    8
#ifndef STFX_SKIP_NAMES_AND_RESETS
static const char *loc_mod_names[NUM_MODS] = {
   "Dry / Wet",
   "Smooth",
   "Shift 1",
   "Level 1",
   "Shift 2",
   "Level 2",
   "Shift 3",
   "Level 3",
};
#endif // !STFX_SKIP_NAMES_AND_RESETS

typedef struct wave_multiplier_delay_info_s {
   st_plugin_info_t base;
} wave_multiplier_delay_info_t;

typedef struct wave_multiplier_delay_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} wave_multiplier_delay_shared_t;

typedef struct wave_multiplier_delay_voice_s {
   st_plugin_voice_t base;
   float   mods[NUM_MODS];
   float   sample_rate;
   float   mod_drywet_cur;
   float   mod_drywet_inc;
   float   mod_shift1_cur;  // time (#frames)
   float   mod_shift1_inc;
   float   mod_level1_cur;
   float   mod_level1_inc;
   float   mod_shift2_cur;
   float   mod_shift2_inc;
   float   mod_level2_cur;
   float   mod_level2_inc;
   float   mod_shift3_cur;
   float   mod_shift3_inc;
   float   mod_level3_cur;
   float   mod_level3_inc;
   StDelay dly[2];  // stereo
   float   mod_time_1_smooth;  // smoothed shifts
   float   mod_time_2_smooth;
   float   mod_time_3_smooth;
} wave_multiplier_delay_voice_t;


static void ST_PLUGIN_API loc_set_sample_rate(st_plugin_voice_t *_voice,
                                              float              _sampleRate
                                              ) {
   ST_PLUGIN_VOICE_CAST(wave_multiplier_delay_voice_t);
   voice->sample_rate = _sampleRate;
}

#ifndef STFX_SKIP_NAMES_AND_RESETS
static const char *ST_PLUGIN_API loc_get_param_name(st_plugin_info_t *_info,
                                                    unsigned int      _paramIdx
                                                    ) {
   (void)_info;
   return loc_param_names[_paramIdx];
}

static float ST_PLUGIN_API loc_get_param_reset(st_plugin_info_t *_info,
                                               unsigned int      _paramIdx
                                               ) {
   (void)_info;
   return loc_param_resets[_paramIdx];
}
#endif // !STFX_SKIP_NAMES_AND_RESETS

static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
                                               unsigned int        _paramIdx
                                               ) {
   ST_PLUGIN_SHARED_CAST(wave_multiplier_delay_shared_t);
   return shared->params[_paramIdx];
}

static void ST_PLUGIN_API loc_set_param_value(st_plugin_shared_t *_shared,
                                              unsigned int        _paramIdx,
                                              float               _value
                                              ) {
   ST_PLUGIN_SHARED_CAST(wave_multiplier_delay_shared_t);
   shared->params[_paramIdx] = _value;
}

#ifndef STFX_SKIP_NAMES_AND_RESETS
static const char *ST_PLUGIN_API loc_get_mod_name(st_plugin_info_t *_info,
                                                  unsigned int      _modIdx
                                                  ) {
   (void)_info;
   return loc_mod_names[_modIdx];
}
#endif // !STFX_SKIP_NAMES_AND_RESETS

static void ST_PLUGIN_API loc_note_on(st_plugin_voice_t  *_voice,
                                      int                 _bGlide,
                                      unsigned char       _note,
                                      float               _vel
                                      ) {
   ST_PLUGIN_VOICE_CAST(wave_multiplier_delay_voice_t);
   (void)_bGlide;
   (void)_note;
   (void)_vel;
   if(!_bGlide)
   {
      memset((void*)voice->mods, 0, sizeof(voice->mods));
      voice->dly[0].reset();
      voice->dly[1].reset();
   }
}

static void ST_PLUGIN_API loc_set_mod_value(st_plugin_voice_t *_voice,
                                            unsigned int       _modIdx,
                                            float              _value,
                                            unsigned           _frameOffset
                                            ) {
   ST_PLUGIN_VOICE_CAST(wave_multiplier_delay_voice_t);
   (void)_frameOffset;
   voice->mods[_modIdx] = _value;
}

static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
                                            unsigned int       _numFrames,
                                            float              _freqHz,
                                            float              _note,
                                            float              _vol,
                                            float              _pan
                                            ) {
   ST_PLUGIN_VOICE_CAST(wave_multiplier_delay_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(wave_multiplier_delay_shared_t);
   (void)_freqHz;
   (void)_note;
   (void)_vol;
   (void)_pan;

   float modDryWet = shared->params[PARAM_DRYWET]   + voice->mods[MOD_DRYWET];
   modDryWet = Dstplugin_clamp(modDryWet, 0.0f, 1.0f);

   float modShift1 = shared->params[PARAM_SHIFT1] + voice->mods[MOD_SHIFT1];
   modShift1 = Dstplugin_clamp(modShift1, 0.0f, 1.0f);

   float modShift2 = shared->params[PARAM_SHIFT2] + voice->mods[MOD_SHIFT2];
   modShift2 = Dstplugin_clamp(modShift2, 0.0f, 1.0f);

   float modShift3 = shared->params[PARAM_SHIFT3] + voice->mods[MOD_SHIFT3];
   modShift3 = Dstplugin_clamp(modShift3, 0.0f, 1.0f);

   float maxMs = (float(ST_DELAY_SIZE) / 88.200f);    // maxMs @ 88.2kHz = ~11.6099773243ms
   if(_numFrames > 0u)
   {
      double modSmooth = shared->params[PARAM_SMOOTH] + voice->mods[MOD_SMOOTH];
      modSmooth = Dstplugin_clamp(modSmooth, 0.0f, 1.0);
      modSmooth  = 1.0 - modSmooth;
      modSmooth *= modSmooth;
      modSmooth *= modSmooth;
      voice->mod_time_1_smooth = (float)(voice->mod_time_1_smooth + (modShift1 - voice->mod_time_1_smooth) * modSmooth);
      voice->mod_time_2_smooth = (float)(voice->mod_time_2_smooth + (modShift2 - voice->mod_time_2_smooth) * modSmooth);
      voice->mod_time_3_smooth = (float)(voice->mod_time_3_smooth + (modShift3 - voice->mod_time_3_smooth) * modSmooth);
   }
   else
   {
      voice->mod_time_1_smooth = modShift1;
      voice->mod_time_2_smooth = modShift2;
      voice->mod_time_3_smooth = modShift3;
   }

   modShift1 = (modShift1 * maxMs * (voice->sample_rate / 1000.0f));
   if(modShift1 >= (float)(ST_DELAY_SIZE - 1u))
      modShift1 = (float)(ST_DELAY_SIZE - 1u);

   modShift2 = (modShift2 * maxMs * (voice->sample_rate / 1000.0f));
   if(modShift2 >= (float)(ST_DELAY_SIZE - 1u))
      modShift2 = (float)(ST_DELAY_SIZE - 1u);

   modShift3 = (modShift3 * maxMs * (voice->sample_rate / 1000.0f));
   if(modShift3 >= (float)(ST_DELAY_SIZE - 1u))
      modShift3 = (float)(ST_DELAY_SIZE - 1u);

   float modLevel1 = shared->params[PARAM_LEVEL1] + voice->mods[MOD_LEVEL1];
   modLevel1 = Dstplugin_clamp(modLevel1, -1.0f, 1.0f);

   float modLevel2 = shared->params[PARAM_LEVEL2] + voice->mods[MOD_LEVEL2];
   modLevel2 = Dstplugin_clamp(modLevel2, -1.0f, 1.0f);

   float modLevel3 = shared->params[PARAM_LEVEL3] + voice->mods[MOD_LEVEL3];
   modLevel3 = Dstplugin_clamp(modLevel3, -1.0f, 1.0f);

   if(_numFrames > 0u)
   {
      // lerp
      float recBlockSize = (1.0f / _numFrames);
      voice->mod_drywet_inc  = (modDryWet  - voice->mod_drywet_cur)  * recBlockSize;
      voice->mod_shift1_inc  = (modShift1  - voice->mod_shift1_cur)  * recBlockSize;
      voice->mod_level1_inc  = (modLevel1  - voice->mod_level1_cur)  * recBlockSize;
      voice->mod_shift2_inc  = (modShift2  - voice->mod_shift2_cur)  * recBlockSize;
      voice->mod_level2_inc  = (modLevel2  - voice->mod_level2_cur)  * recBlockSize;
      voice->mod_shift3_inc  = (modShift3  - voice->mod_shift3_cur)  * recBlockSize;
      voice->mod_level3_inc  = (modLevel3  - voice->mod_level3_cur)  * recBlockSize;
   }
   else
   {
      // initial params/modulation (first block, not rendered)
      voice->mod_drywet_cur  = modDryWet;
      voice->mod_drywet_inc  = 0.0f;
      voice->mod_shift1_cur  = modShift1;
      voice->mod_shift1_inc  = 0.0f;
      voice->mod_level1_cur  = modLevel1;
      voice->mod_level1_inc  = 0.0f;
      voice->mod_shift2_cur  = modShift2;
      voice->mod_shift2_inc  = 0.0f;
      voice->mod_level2_cur  = modLevel2;
      voice->mod_level2_inc  = 0.0f;
      voice->mod_shift3_cur  = modShift3;
      voice->mod_shift3_inc  = 0.0f;
      voice->mod_level3_cur  = modLevel3;
      voice->mod_level3_inc  = 0.0f;
   }
}

static void ST_PLUGIN_API loc_process_replace(st_plugin_voice_t  *_voice,
                                              int                 _bMonoIn,
                                              const float        *_samplesIn,
                                              float              *_samplesOut,
                                              unsigned int        _numFrames
                                              ) {
   // Ring modulate at (modulated) note frequency
   ST_PLUGIN_VOICE_CAST(wave_multiplier_delay_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(wave_multiplier_delay_shared_t);

   unsigned int k = 0u;

   for(unsigned int i = 0u; i < _numFrames; i++)
   {
      for(unsigned int ch = 0u; ch < 2u; ch++)
      {
         float l = _samplesIn[k + ch];

         float out1 = voice->dly[ch].readLinear(voice->mod_shift1_cur);
         float out2 = voice->dly[ch].readLinear(voice->mod_shift2_cur);
         float out3 = voice->dly[ch].readLinear(voice->mod_shift3_cur);

         float outL = out1 * voice->mod_level1_cur;
         outL += out2 * voice->mod_level2_cur;
         outL += out3 * voice->mod_level3_cur;

         outL = l + (outL - l) * voice->mod_drywet_cur;

         voice->dly[ch].push(l, 0.0f/*fb*/);

         _samplesOut[k + ch] = outL;
      }

      // Next frame
      k += 2u;
      voice->mod_drywet_cur  += voice->mod_drywet_inc;
      voice->mod_shift1_cur  += voice->mod_shift1_inc;
      voice->mod_level1_cur  += voice->mod_level1_inc;
      voice->mod_shift2_cur  += voice->mod_shift2_inc;
      voice->mod_level2_cur  += voice->mod_level2_inc;
      voice->mod_shift3_cur  += voice->mod_shift3_inc;
      voice->mod_level3_cur  += voice->mod_level3_inc;
   }

}

static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   wave_multiplier_delay_shared_t *ret = (wave_multiplier_delay_shared_t *)malloc(sizeof(wave_multiplier_delay_shared_t));
   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));
      ret->base.info  = _info;
#ifndef STFX_SKIP_NAMES_AND_RESETS
      memcpy((void*)ret->params, (void*)loc_param_resets, NUM_PARAMS * sizeof(float));
#endif // !STFX_SKIP_NAMES_AND_RESETS
   }
   return &ret->base;
}

static void ST_PLUGIN_API loc_shared_delete(st_plugin_shared_t *_shared) {
   free((void*)_shared);
}

static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   wave_multiplier_delay_voice_t *ret = (wave_multiplier_delay_voice_t *)malloc(sizeof(wave_multiplier_delay_voice_t));
   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));
      ret->base.info = _info;
   }
   return &ret->base;
}

static void ST_PLUGIN_API loc_voice_delete(st_plugin_voice_t *_voice) {
   free((void*)_voice);
}

static void ST_PLUGIN_API loc_plugin_exit(st_plugin_info_t *_info) {
   free((void*)_info);
}

extern "C" {
st_plugin_info_t *wave_multiplier_delay_init(void) {
   wave_multiplier_delay_info_t *ret = NULL;

   ret = (wave_multiplier_delay_info_t *)malloc(sizeof(wave_multiplier_delay_info_t));

   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));

      ret->base.api_version = ST_PLUGIN_API_VERSION;
      ret->base.id          = "bsp wave_multiplier_delay";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "wave_multiplier_delay";
      ret->base.short_name  = "wave_multiplier_delay";
      ret->base.flags       = ST_PLUGIN_FLAG_FX;
      ret->base.category    = ST_PLUGIN_CAT_CHORUS;
      ret->base.num_params  = NUM_PARAMS;
      ret->base.num_mods    = NUM_MODS;

      ret->base.shared_new         = &loc_shared_new;
      ret->base.shared_delete      = &loc_shared_delete;
      ret->base.voice_new          = &loc_voice_new;
      ret->base.voice_delete       = &loc_voice_delete;
#ifndef STFX_SKIP_NAMES_AND_RESETS
      ret->base.get_param_name     = &loc_get_param_name;
      ret->base.get_param_reset    = &loc_get_param_reset;
#endif // !STFX_SKIP_NAMES_AND_RESETS
      ret->base.get_param_value    = &loc_get_param_value;
      ret->base.set_param_value    = &loc_set_param_value;
#ifndef STFX_SKIP_NAMES_AND_RESETS
      ret->base.get_mod_name       = &loc_get_mod_name;
#endif // !STFX_SKIP_NAMES_AND_RESETS
      ret->base.set_sample_rate    = &loc_set_sample_rate;
      ret->base.note_on            = &loc_note_on;
      ret->base.set_mod_value      = &loc_set_mod_value;
      ret->base.prepare_block      = &loc_prepare_block;
      ret->base.process_replace    = &loc_process_replace;
      ret->base.plugin_exit        = &loc_plugin_exit;
   }

   return &ret->base;
}
} // extern "C"

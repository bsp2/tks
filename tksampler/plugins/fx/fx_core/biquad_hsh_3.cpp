...
typedef struct biquad_hsh_3_info_s {
   st_plugin_info_t base;
} biquad_hsh_3_info_t;

typedef struct biquad_hsh_3_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} biquad_hsh_3_shared_t;

typedef struct biquad_hsh_3_voice_s {
...
} biquad_hsh_3_voice_t;


static void ST_PLUGIN_API loc_set_sample_rate(st_plugin_voice_t *_voice,
                                              float              _sampleRate
                                              ) {
   ST_PLUGIN_VOICE_CAST(biquad_hsh_3_voice_t);
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(biquad_hsh_3_shared_t);
...
static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(biquad_hsh_3_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(biquad_hsh_3_shared_t);
   (void)_freqHz;
   (void)_note;
   (void)_vol;
   (void)_pan;
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   biquad_hsh_3_shared_t *ret = (biquad_hsh_3_shared_t *)malloc(sizeof(biquad_hsh_3_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   biquad_hsh_3_voice_t *ret = (biquad_hsh_3_voice_t *)malloc(sizeof(biquad_hsh_3_voice_t));
   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));
      ret->base.info = _info;
...
extern "C" {
st_plugin_info_t *biquad_hsh_3_init(void) {
   biquad_hsh_3_info_t *ret = NULL;

   ret = (biquad_hsh_3_info_t *)malloc(sizeof(biquad_hsh_3_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp biquad hsh 3";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "biquad high shelf 3";
      ret->base.short_name  = "bq hsh 3";

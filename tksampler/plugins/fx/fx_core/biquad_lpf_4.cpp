...
typedef struct biquad_lpf_4_info_s {
   st_plugin_info_t base;
} biquad_lpf_4_info_t;

typedef struct biquad_lpf_4_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} biquad_lpf_4_shared_t;

typedef struct biquad_lpf_4_voice_s {
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(biquad_lpf_4_shared_t);
...
static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(biquad_lpf_4_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(biquad_lpf_4_shared_t);
   (void)_freqHz;
   (void)_note;
   (void)_vol;
   (void)_pan;
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   biquad_lpf_4_shared_t *ret = (biquad_lpf_4_shared_t *)malloc(sizeof(biquad_lpf_4_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   biquad_lpf_4_voice_t *ret = (biquad_lpf_4_voice_t *)malloc(sizeof(biquad_lpf_4_voice_t));
   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));
      ret->base.info = _info;
...
extern "C" {
st_plugin_info_t *biquad_lpf_4_init(void) {
   biquad_lpf_4_info_t *ret = NULL;

   ret = (biquad_lpf_4_info_t *)malloc(sizeof(biquad_lpf_4_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp biquad lpf 4";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "biquad low pass filter 4";
      ret->base.short_name  = "bq lpf 4";

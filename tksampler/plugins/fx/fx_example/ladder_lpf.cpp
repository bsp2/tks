...
typedef struct ladder_lpf_info_s {
   st_plugin_info_t base;
} ladder_lpf_info_t;

typedef struct ladder_lpf_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} ladder_lpf_shared_t;

typedef struct ladder_lpf_voice_s {
...
static void ST_PLUGIN_API loc_set_sample_rate(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(ladder_lpf_voice_t);
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(ladder_lpf_shared_t);
...
static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(ladder_lpf_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(ladder_lpf_shared_t);
   (void)_freqHz;
   (void)_note;
   (void)_vol;
   (void)_pan;
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   ladder_lpf_shared_t *ret = (ladder_lpf_shared_t *)malloc(sizeof(ladder_lpf_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   ladder_lpf_voice_t *ret = (ladder_lpf_voice_t *)malloc(sizeof(ladder_lpf_voice_t));
   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));
      ret->base.info = _info;
...
extern "C" {
st_plugin_info_t *ladder_lpf_init(void) {
   ladder_lpf_info_t *ret = NULL;

   ret = (ladder_lpf_info_t *)malloc(sizeof(ladder_lpf_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp ladder lpf";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "ladder low pass filter";
      ret->base.short_name  = "ladder lpf";

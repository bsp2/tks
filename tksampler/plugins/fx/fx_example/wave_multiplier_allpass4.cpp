// ----
// ---- file   : wave_multiplier_allpass4.cpp
...
#define PARAM_FREQ_BASE   6
#define PARAM_FREQ_RAND   7
#define NUM_PARAMS        8
...
#define MOD_FREQ_BASE   6
#define MOD_FREQ_RAND   7
#define NUM_MODS        8
...
typedef struct wave_multiplier_allpass4_info_s {
   st_plugin_info_t base;
   float sintbl[65536];
   unsigned int lfsr_state;
} wave_multiplier_allpass4_info_t;

typedef struct wave_multiplier_allpass4_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} wave_multiplier_allpass4_shared_t;

typedef struct wave_multiplier_allpass4_part_s {
   float angle;
   float speed;
   float          speed_r;
   float          level_r;
...
} wave_multiplier_allpass4_part_t;

typedef struct wave_multiplier_allpass4_voice_s {
   ...
   wave_multiplier_allpass4_part_t parts[MAX_PARTS];
} wave_multiplier_allpass4_voice_t;


static void loc_init_sintbl(wave_multiplier_allpass4_info_t *_info) {
...
static void ST_PLUGIN_API loc_set_sample_rate(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(wave_multiplier_allpass4_voice_t);
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(wave_multiplier_allpass4_shared_t);
...
static void ST_PLUGIN_API loc_note_on(st_plugin_voice_t  *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(wave_multiplier_allpass4_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(wave_multiplier_allpass4_shared_t);
   ST_PLUGIN_VOICE_INFO_CAST(wave_multiplier_allpass4_info_t);
   (void)_bGlide;
   (void)_note;
   (void)_vel;
   if(!_bGlide)
   {
      ...
      {
         wave_multiplier_allpass4_part_t *part = &voice->parts[partIdx];
...
static void ST_PLUGIN_API loc_prepare_block(st_plugin_voice_t *_voice,
   ...
   for(unsigned int partIdx = 0u; partIdx < MAX_PARTS; partIdx++)
   {
      ...
      part->speed = (16.0f * (shared->params[PARAM_SPEED_BASE] + voice->mods[MOD_SPEED_BASE] + (shared->params[PARAM_SPEED_RAND] + voice->mods[MOD_SPEED_RAND]) * part->speed_r));

      part->freq_base = ((shared->params[PARAM_FREQ_BASE]-0.5f) * 2.0f) + voice->mods[MOD_FREQ_BASE];
      part->freq_rand = part->freq_r * (shared->params[PARAM_FREQ_RAND] + voice->mods[MOD_FREQ_RAND]);
   ...
   for(unsigned int i = 0u; i < _numFrames; i++)
   {
      for(unsigned int ch = 0u; ch < 2u; ch++)
      {
         ...
         for(unsigned int partIdx = 0u; partIdx < MAX_PARTS/*voice->num_parts*/; partIdx++)
         {
            ...
            float freq = part->freq_base + part->freq_rand * sinAng;
            float flt = l;
            for(unsigned int poleIdx = 0u; poleIdx < NUM_POLES; poleIdx++)
            {
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   wave_multiplier_allpass4_shared_t *ret = (wave_multiplier_allpass4_shared_t *)malloc(sizeof(wave_multiplier_allpass4_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   wave_multiplier_allpass4_voice_t *ret = (wave_multiplier_allpass4_voice_t *)malloc(sizeof(wave_multiplier_allpass4_voice_t));
   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));
      ret->base.info = _info;
...
extern "C" {
st_plugin_info_t *wave_multiplier_allpass4_init(void) {
   wave_multiplier_allpass4_info_t *ret = NULL;

   ret = (wave_multiplier_allpass4_info_t *)malloc(sizeof(wave_multiplier_allpass4_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp wave_multiplier_allpass4";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "wave_multiplier_allpass4";
      ret->base.short_name  = "wave_multiplier_allpass4";

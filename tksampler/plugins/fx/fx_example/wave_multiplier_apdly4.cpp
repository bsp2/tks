...
} wave_multiplier_apdly4_info_t;

typedef struct wave_multiplier_apdly4_shared_s {
   st_plugin_shared_t base;
   float params[NUM_PARAMS];
} wave_multiplier_apdly4_shared_t;

typedef struct wave_multiplier_apdly4_part_s {
   struct {
      float angle;
      float speed;
      float speed_r;
   ...
   StDelay  dly[2];
} wave_multiplier_apdly4_part_t;

typedef struct wave_multiplier_apdly4_voice_s {
   ...
   float   fb;
   wave_multiplier_apdly4_part_t parts[MAX_PARTS];
} wave_multiplier_apdly4_voice_t;


static void loc_init_sintbl(wave_multiplier_apdly4_info_t *_info) {
...
static void ST_PLUGIN_API loc_set_sample_rate(st_plugin_voice_t *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(wave_multiplier_apdly4_voice_t);
...
static float ST_PLUGIN_API loc_get_param_value(st_plugin_shared_t *_shared,
   ...
   ST_PLUGIN_SHARED_CAST(wave_multiplier_apdly4_shared_t);
...
static void ST_PLUGIN_API loc_note_on(st_plugin_voice_t  *_voice,
   ...
   ST_PLUGIN_VOICE_CAST(wave_multiplier_apdly4_voice_t);
   ST_PLUGIN_VOICE_SHARED_CAST(wave_multiplier_apdly4_shared_t);
   ST_PLUGIN_VOICE_INFO_CAST(wave_multiplier_apdly4_info_t);
   (void)_bGlide;
   (void)_note;
   (void)_vel;
   if(!_bGlide)
   {
      ...
      {
         wave_multiplier_apdly4_part_t *part = &voice->parts[partIdx];
...
static st_plugin_shared_t *ST_PLUGIN_API loc_shared_new(st_plugin_info_t *_info) {
   wave_multiplier_apdly4_shared_t *ret = (wave_multiplier_apdly4_shared_t *)malloc(sizeof(wave_multiplier_apdly4_shared_t));
   if(NULL != ret)
   {
...
static st_plugin_voice_t *ST_PLUGIN_API loc_voice_new(st_plugin_info_t *_info, unsigned int _voiceIdx) {
   (void)_voiceIdx;
   wave_multiplier_apdly4_voice_t *ret = (wave_multiplier_apdly4_voice_t *)malloc(sizeof(wave_multiplier_apdly4_voice_t));
   if(NULL != ret)
   {
      memset((void*)ret, 0, sizeof(*ret));
      ret->base.info = _info;
...
extern "C" {
st_plugin_info_t *wave_multiplier_apdly4_init(void) {
   wave_multiplier_apdly4_info_t *ret = NULL;

   ret = (wave_multiplier_apdly4_info_t *)malloc(sizeof(wave_multiplier_apdly4_info_t));
   ...
   if(NULL != ret)
   {
      ...
      ret->base.id          = "bsp wave_multiplier_apdly4";  // unique id. don't change this in future builds.
      ret->base.author      = "bsp";
      ret->base.name        = "wave_multiplier_apdly4";
      ret->base.short_name  = "wave_multiplier_apdly4";

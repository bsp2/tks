...
- `-DTKSAMPLER_PLUGINS_ONLY`        (disable sample playback)
- `-DTKSAMPLER_SKIP_TUNING_TABLES`  (disable tuning tables)
- `-DTKSAMPLER_SKIP_MUTEX_GROUPS`   (skip sample zone mutex groups)
- `-DTKSAMPLER_SKIP_RANGE`          (skip all sample zone ranges)
...
- `-DTKSAMPLER_SKIP_LOOP_OPS`       (skip sample loop table ops)
- `-DTKSAMPLER_SKIP_TIMED_LOOP`     (skip timed loops + oneshot loopsteps)

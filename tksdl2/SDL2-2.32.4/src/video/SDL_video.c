...
// [bsp] rebind GL context
void SDL_GL_Context_Rebind(SDL_Window *window) {
   int retval;
   ...
   // printf("xxx SDL_RebindGLContext called\n");
   context = SDL_GL_GetCurrentContext();
   retval = _this->GL_MakeCurrent(_this, window, context);
   if (retval == 0) {
      _this->current_glwin = window;
...
void SDL_GL_Context_Push(SDL_Window *window)
{
   if (!_this) {
      SDL_UninitializedVideo();
      return;// NULL;
...
void SDL_GL_Context_Pop(SDL_Window *window)
{
   ...
   _this->GL_PopContext(_this, window);
}

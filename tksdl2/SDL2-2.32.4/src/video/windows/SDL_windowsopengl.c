...
int WIN_GL_LoadLibrary(_THIS, const char *path)
{
    ...
    _this->gl_data->wglGetCurrentDC = (HDC(WINAPI *)(void))         // [bsp]
       SDL_LoadFunction(handle, "wglGetCurrentDC");                 // [bsp]
    _this->gl_data->wglGetCurrentContext = (HGLRC(WINAPI *)(void))  // [bsp]
       SDL_LoadFunction(handle, "wglGetCurrentContext");            // [bsp]
...
// [bsp] push GL context before/after repaint to preserve VST plugin GL context
static HDC loc_old_hdc = 0;
static HGLRC loc_old_hglrc = 0;
void WIN_GL_PushContext(_THIS, SDL_Window *window)
{
   HDC hdc = _this->gl_data->wglGetCurrentDC();
   HGLRC hglrc = _this->gl_data->wglGetCurrentContext();
...
#ifdef DEBUGL_GL_CONTEXT
      printf("xxx WIN_GL_PushContext: old HDC=%p hglrc=%p\n", loc_old_hdc, loc_old_hglrc);
#endif // DEBUGL_GL_CONTEXT
      // printf("xxx SDL_PushGLContext called\n");
      WIN_GL_MakeCurrent(_this, window, SDL_GL_GetCurrentContext());
   }
}

// [bsp] restore previous GL context (after repaint, to preserve VST plugin GL context)
void WIN_GL_PopContext(_THIS, SDL_Window *window)
{
   (void)window;
   if(0 != loc_old_hglrc)
   {
      // printf("xxx SDL_PushGLContext called\n");
#ifdef DEBUGL_GL_CONTEXT
      printf("xxx WIN_GL_PopContext: restore HDC=%p hglrc=%p\n", loc_old_hdc, loc_old_hglrc);
#endif // DEBUGL_GL_CONTEXT
      _this->gl_data->wglMakeCurrent(loc_old_hdc, loc_old_hglrc);
      loc_old_hdc = 0;
      loc_old_hglrc = 0;
   }
}

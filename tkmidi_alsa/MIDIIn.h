...
///          14Apr2013, 04Feb2014, 05Feb2014, 09Feb2014, 23Aug2014, 11Feb2015, 29Mar2019
///          12Nov2020, 22Jul2023, 01Aug2023
///
///
///
...
#define MIDIIN_BLOCKSIZE 32768 // Maximum size of sysex message
#define MIDIIN_NUMBUFFERS    (32)  // Number of SysEx buffers
#define MIDIIN_MAXEVENTS 1024 // Size of RecordedMIDIEvent ring buffer
class MIDIIn;
...
YC class RecordedMIDIEvent : public YAC_Object {
  public:
   sUI millisec;    // Since program startup
   sBool b_long;    // true==long (sysex) message (data.ptr), size), false=short msg (data.msg)
   union {
      sU8 *ptr;     // b_long==true, Reference to MIDIIn::midi_buffer_data element
      sU32 msg32;   // b_long==false, e.g. 0x007b48b1
      sU8  msg[4];  // b_long==false, e.g. 0xB1 0x48 0x7b
   } data;
   sUI  size;        // Number of data bytes (only valid when b_long == true)
   sU8  midimap_event_type; // see MIDIMapEventType.tks
...
public:
  ...
  public:
   sUI  YAC_VCALL yacArrayGetNumElements (void); // return message size (#bytes)
   void YAC_VCALL yacArrayGet            (void *_context, sUI _index, YAC_Value *_r); // return message byte
...
public:
   ...
   sBool b_running;  // YAC_TRUE while reader thread is running
   sBool b_thread_busy; // YAC_TRUE while reader thread processes input
...
public:
   ...
   */
   ...
   /* @method getNumEvent:int
     Query number of recorded input events

    @return Number of recorded input events
   */
   YM sUI         _getNumEvents  (void); // Get number of queued RecordedMIDIEvents
   ...
   */
   YM void GetDeviceNameByIdx (sUI _idx, YAC_Value *_r);

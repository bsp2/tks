...
#define Dopprintf if(0);else yac_host->printf
...
extern void tkvst2_macos_create_window_view (tkvst2_window_view_t *_winView, int _w, int _h, const char *_title);
extern void tkvst2_macos_get_window_geometry (tkvst2_window_view_t *_winView, int *x, int *y, int *w, int *h);
extern void tkvst2_macos_set_window_geometry (tkvst2_window_view_t *_winView, int x, int y, int w, int h);
extern void tkvst2_macos_close_window_view (tkvst2_window_view_t *_winView);
extern void tkvst2_macos_setvisible_window_view (tkvst2_window_view_t *_winView, int bHide);
extern int tkvst2_macos_isvisible_window_view (tkvst2_window_view_t *_winView);
extern void tkvst2_macos_tofront_window_view (tkvst2_window_view_t *_winView);
extern void tkvst2_macos_update_window_view (tkvst2_window_view_t *_winView);
...
static TKVST2Window *CreateVST2Window (VST2Plugin *thiz);

#if 0
static TKVST2Window *FindVST2WindowByNSWindow(NSWindow *_nsWindow) {
...
void RemoveVST2WindowByNSView(void *_nsView) {
   // called by VST2View::windowWillClose

   yac_host->printf("[dbg] tkvst2::RemoveVST2WindowByHandle: nsView=%p\n", _nsView);
   ...
   while(NULL != vw)
   {
      if(vw->window_view.nsView == _nsView)
      {
         // Remember window position and size
         {
            ...
            int x = 0, y = 0, w = 0, h = 0;
            tkvst2_macos_get_window_geometry(&vw->window_view, &x, &y, &w, &h);
            vw->plugin->ui_window_x = x;
            vw->plugin->ui_window_y = y;
            vw->plugin->ui_window_w = w;
            vw->plugin->ui_window_h = h;
            yac_host->printf("[dbg] tkvst2::RemoveVST2WindowByHandle: geo=(%d; %d; %d; %d)\n",
                             x, y, w, h);
         }
         ...
         yac_host->printf("[dbg] tkvst2::RemoveVST2WindowByHandle: window nsView=0x%p destroyed\n", _nsView);
...
static VstIntPtr VSTCALLBACK HostCallback (AEffect* effect, VstInt32 opcode, VstInt32 index, VstIntPtr value, void* ptr, float opt) {
   ...
   switch(opcode)
   {
      ...
      case audioMasterUpdateDisplay:
      {
         ...
         if(NULL != plugin)
         {
            ...
            if(NULL != vw)
            {
               tkvst2_macos_update_window_view(&vw->window_view);
            }
         }
         result = 1;
      }
   ...
   // yac_host->printf("xxx HostCallback: return result=%u\n", result);
...
void VST2Plugin::callOnMacOSKeyDown(sUI _keyCode) {
   void *f = tkvst2_audio_master_signal_funs[VST2PLUGIN_SIGNAL_ONMACOSKEYDOWN];
...
#else
#error this version of tkvst is for macOS
#endif // YAC_MACOS
...
static TKVST2Window *CreateVST2Window(VST2Plugin *thiz) {
   ...
   yac_host->printf("[dbg] tkvst2::CreateVST2Window: ENTER.\n");

   BuildWindowTitle(thiz);

   yac_host->printf("[dbg] tkvst2::CreateVST2Window: 5a call effEditGetRect\n");
   ...
   yac_host->printf("[dbg] tkvst2::CreateVST2Window: 5b calleffEditGetRect returned\n");

   if(eRect)
   {
      yac_host->printf("[dbg] tkvst2::CreateVST2Window: 6\n");
      ...
      yac_host->printf("[dbg] tkvst2::CreateVST2Window: 7 effEditGetRect indicates size=(%d; %d)\n", width, height);
      ...
      // will be overwritten by initial windowDidMove() callback (0;0)
      sSI uiWindowX = thiz->ui_window_x;
      sSI uiWindowY = thiz->ui_window_y;

      vw = new(std::nothrow) TKVST2Window;
      tkvst2_window_view_t windowView = {thiz, vw, NULL, NULL};
      tkvst2_macos_create_window_view(&windowView, width, height, init_windowTitle);
      ...
      yac_host->printf("[dbg] tkvst2::CreateVST2Window: calling effEditOpen\n");
      thiz->beginDispatch();
      VstIntPtr ip = thiz->effect->dispatcher(thiz->effect, effEditOpen, 0, 0, vw->window_view.nsView, 0.0f);
      (void)ip;
      // bOK = sSI(ip);
      ...
      if(thiz->ui_window_w >= 100)
      {
         ...
         int x = thiz->ui_window_x;
         int y = thiz->ui_window_y;
         int w = thiz->ui_window_w;
         int h = thiz->ui_window_h;
         yac_host->printf("[dbg] tkvst2::CreateVST2Window: restore geo=(%d; %d; %d; %d)\n",
                          thiz->ui_window_x, thiz->ui_window_y, thiz->ui_window_w, thiz->ui_window_h
                          );
         tkvst2_macos_set_window_geometry(&vw->window_view, x, y, w, h);
      }
      else
      {
         tkvst2_macos_get_window_geometry(&vw->window_view, &thiz->ui_window_x, &thiz->ui_window_y, &thiz->ui_window_w, &thiz->ui_window_h);
         yac_host->printf("[dbg] tkvst2::CreateVST2Window: query geo=(%d; %d; %d; %d)\n",
                          thiz->ui_window_x, thiz->ui_window_y, thiz->ui_window_w, thiz->ui_window_h
                          );
      }
   ...
   {
      yac_host->printf("[---] tkvst2::CreateVST2Window: effEditGetRect failed\n");
   }
...
void VST2Plugin::showEditor(void) {
   if(NULL != effect)
   {
      ...
      else
      {
         ...
         // To front
         yac_host->printf("[dbg] VST2Plugin::showEditor: window exists, calling setVisible\n");
         tkvst2_macos_setvisible_window_view(&vw->window_view, YAC_TRUE);
      }
...
void VST2Plugin::hideEditor(void) {
   ...
   if(NULL != vw)
   {
      yac_host->printf("[dbg] VST2Plugin::hideEditor: call setVisible\n");
      tkvst2_macos_setvisible_window_view(&vw->window_view, YAC_FALSE);
   }
}
...
void VST2Plugin::closeEditor(void) {
   ...
   if(0u != uid)  // not the template instance ?
   {
      ...
      if(NULL != vw)
      {
         ...
         tkvst2_macos_close_window_view(&vw->window_view);
      }
   }
}
...
sBool VST2Plugin::isEditorVisible(void) {
   if(NULL != effect)
   {
      ...
      if(NULL != vw)
      {
         return tkvst2_macos_isvisible_window_view(&vw->window_view);
      }
   }
   return YAC_FALSE;
}
   ...
   while(NULL != vw)
   {
      if(NULL != vw->plugin)
      {
         if(-1 != vw->plugin->queued_resize_w) {
            ...
            {
               yac_host->printf("[dbg] VST2Plugin::HandleQueuedWindowResize: vw=%p queued_resize=(%d; %d)\n", vw, vw->plugin->queued_resize_w, vw->plugin->queued_resize_h);

               vw->plugin->ui_window_w = vw->plugin->queued_resize_w;
               vw->plugin->ui_window_h = vw->plugin->queued_resize_h;

               // PSP Echo resize-larger-than-screen workaround
               //  (todo) query actual screen size
#if 0
               if(vw->plugin->ui_window_w > 1920)
                  vw->plugin->ui_window_w = 1920;
               if(vw->plugin->ui_window_h > 1100)
                  vw->plugin->ui_window_h = 1100;
#endif

               // (todo) resize window
               int x = vw->plugin->ui_window_x;
               int y = vw->plugin->ui_window_y;
               int w = vw->plugin->ui_window_w;
               int h = vw->plugin->ui_window_h;
               tkvst2_macos_set_window_geometry(&vw->window_view, x, y, w, h);
...
void VST2Plugin::windowToFront(void) {
   // macOS-only workaround for Arturia/PSP InfiniStrip host-redraw issues
   TKVST2Window *vw = FindVST2WindowByPlugin(this, 1/*bLock*/);
   if(NULL != vw)
   {
      tkvst2_macos_tofront_window_view(&vw->window_view);
   }
}

...
void _PortAudio::lazyEnumerateDevices(void) {
   ...
   while( 0 == snd_card_next(&cardIdx) && cardIdx >= 0 )
   {
      ...
      {
         // Unable to open card :(
         yac_host->printf("[---] PortAudio<alsa>::enumerateDevices: unable to open card %s\n", alsaCardName);
         continue;
      }
      ...
      while( 0 == snd_ctl_pcm_next_device(ctl, &devIdx) && devIdx >= 0 )
      {
         ...
         // Open device and query rate/#channels
         {
            ...
            if(err >= 0)
            {
               ...
               {
                  yac_host->printf("[~~~] PortAudio<alsa>::enumerateDevices: snd_pcm_get_params() failed. err=%d(\"%s\")\n", err, snd_strerror(err));
               }
#endif
               ...
               // Find default sample format
               {
                  ...
                  {
                     yac_host->printf("[~~~] PortAudio<alsa>::enumerateDevices: unsupported sample format (hwName=\"%s\")\n", hwName);
                  }
               }
               ...
               if(0u == sampleRate)
               {
                  ...
                  {
                     yac_host->printf("[~~~] PortAudio<alsa>::enumerateDevices: snd_pcm_hw_params_get_rate_min() failed. err=%d(\"%s\")\n", err, snd_strerror(err));
                  }
               }
               ...
               if(0u == sampleRate)
               {
                  ...
                  {
                     yac_host->printf("[~~~] PortAudio<alsa>::enumerateDevices: snd_pcm_hw_params_get_rate_max() failed. err=%d(\"%s\")\n", err, snd_strerror(err));
                  }
               }
               ...
               // (note) fails for most devices (but is expected behaviour according to docs)
               {
                  ...
                  {
                     yac_host->printf("[~~~] PortAudio<alsa>::enumerateDevices: snd_pcm_hw_params_get_rate_numden() failed. err=%d(\"%s\")\n", err, snd_strerror(err));
                  }
               }
#endif
               ...
               if(0u == bufferSizeMin)
               {
                  ...
                  if(err < 0)
                  {
                     yac_host->printf("[~~~] PortAudio<alsa>::enumerateDevices: snd_pcm_hw_params_get_buffer_size_min() failed. err=%d(\"%s\")\n", err, snd_strerror(err));
                  }
                  else
                  {
                     yac_host->printf("[trc] PortAudio<alsa>::enumerateDevices: bufferSizeMin=%u\n", bufferSizeMin);
                  }
               }
               ...
               if(0u == bufferSizeMax)
               {
                  ...
                  if(err < 0)
                  {
                     yac_host->printf("[~~~] PortAudio<alsa>::enumerateDevices: snd_pcm_hw_params_get_buffer_size_max() failed. err=%d(\"%s\")\n", err, snd_strerror(err));
                  }
                  else
                  {
                     yac_host->printf("[trc] PortAudio<alsa>::enumerateDevices: bufferSizeMax=%u\n", bufferSizeMax);
                  }
               }
               ...
               if(0u == periodSizeMin)
               {
                  ...
                  if(err < 0)
                  {
                     yac_host->printf("[~~~] PortAudio<alsa>::enumerateDevices: snd_pcm_hw_params_get_period_size_min() failed. err=%d(\"%s\")\n", err, snd_strerror(err));
                  }
               }
               ...
               if(0u == periodSizeMax)
               {
                  ...
                  if(err < 0)
                  {
                     yac_host->printf("[~~~] PortAudio<alsa>::enumerateDevices: snd_pcm_hw_params_get_period_size_max() failed. err=%d(\"%s\")\n", err, snd_strerror(err));
                  }
               }
#endif
            ...
            {
               yac_host->printf("[~~~] PortAudio<alsa>::enumerateDevices: snd_pcm_open(hwName=\"%s\") failed. err=%d(\"%s\")\n", hwName, err, snd_strerror(err));
            }
         }
      ...
      if(YAC_CHK(_outParams, clid_PaStreamParameters))
      {
         ...
         if(YAC_VALID(_streamCbk)) //if(YAC_CHK(_streamCbk, YAC_CLID_FUNCTION))
         {
            if(YAC_CHK(_retStream, clid_PaStream))
            {
               ...
               {
                  yac_host->printf("[---] PortAudio<alsa>::openStream: initStream() failed.\n");
               }
...
sSI _PortAudio::_GetSampleSize(sSI _format) {
   ...
   yac_host->printf("[~~~] PortAudio<alsa>::GetSampleSize: unsupported format %d\n", _format);
   return 0;
}

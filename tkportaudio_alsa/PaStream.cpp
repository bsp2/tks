...
static sBool loc_perform_hack(snd_pcm_t *handle, sUI _minBuf, sUI _maxBuf)
{
   ...
   // if(NULL != dlHandle)
   {
      ...
      if(NULL != snd_pcm_hw_set_hack_params)
      {
         ...
         yac_host->printf("[dbg] PaStream<alsa>: set hack params: %d(\"%s\")\n", ret, strerror(-ret));

         return (0 == ret);
      }
      else
      {
         yac_host->printf("[~~~] PaStream<alsa>: no pandora hack in libasound.\n");
      }
   ...
   // else
   // {
   //    yac_host->printf("[~~~] PaStream<alsa>: failed to dlopen() libasound.\n");
   // }
   ...
   while(*(volatile sBool*)&st->b_running)
   {
      ...
      // if(0u == sleepLeft)
      {
         ...
         if(st->b_running)
         {
            ...
            Dprintf("[trc] tkportaudio<alsa>:thread: numInCh=%u numOutCh=%u inAvail=%u outAvail=%u\n", st->num_in_ch, st->num_out_ch, inAvail, outAvail);
            sUI maxRingAvail = st->num_periods * st->period_size;
            Dprintf("[trc] tkportaudio<alsa>:thread: numInCh=%u numOutCh=%u inAvail=%u outAvail=%u maxRingAvail=%u\n", st->num_in_ch, st->num_out_ch, inAvail, outAvail, maxRingAvail);
                ...
                )
            {
               ...
               if(bVerbose)
               {
                  Dprintf("[trc] tkportaudio<alsa>:thread: periodCount=%u numInCh=%u numOutCh=%u inAvail=%u outAvail=%u periodSize=%u\n", periodCount, st->num_in_ch, st->num_out_ch, inAvail, outAvail, st->period_size);
               }
               ...
               if(st->num_in_ch > 0u)
               {
                  ...
                  else
                  {
                     ...
                     Dprintf("[~~~] tkportaudio<alsa>:thread: input underrun: inAvail=%u req=%u. filling with zero..\n", inAvail, st->period_size);
                  }
               }
            ...
            {
#if 0
               Dprintf("[dbg] tkportaudio<alsa>:thread: pcmState=SND_PCM_STATE_RUNNING, async_cbk_count_out=%u avgAvail=%.3f\n",
                       async_cbk_count_out,
                       st->dbg_avg_avail
                       );
#endif
            ...
            {
               yac_host->printf("[~~~] tkportaudio<alsa>:thread: detected SND_PCM_STATE_XRUN, (re-)preparing stream..\n");

               err = snd_pcm_prepare(st->out_handle);

               if(err < 0)
               {
                  yac_host->printf("[~~~] tkportaudio<alsa>:thread: snd_pcm_prepare() failed. err=%d(\"%s\").\n",
                                   err,
                                   snd_strerror(err)
                                   );
               }
            ...
            {
               yac_host->printf("[dbg] tkportaudio<alsa>:thread: detected SND_PCM_STATE_PREPARED, (re-)starting stream..\n");
               ...
               Dprintf("[trc] tkportaudio<alsa>:thread: SND_PCM_STATE_PREPARED: snd_pcm_writei() returned\n");
            }
            else if(SND_PCM_STATE_PAUSED == pcmState)
            {
               if(st->b_active)
               {
                  ...
                  if(err < 0)
                  {
                     yac_host->printf("[~~~] tkportaudio<alsa>:thread: snd_pcm_start() failed. err=%d(\"%s\").\n",
                                      err,
                                      snd_strerror(err)
                                      );
                  }
            ...
            {
#ifdef USE_ASYNC
               Dprintf("[dbg] tkportaudio<alsa>:thread: pcmState=%d, async_cbk_count_out=%u\n", pcmState, async_cbk_count_out);
#else
               Dprintf("[dbg] tkportaudio<alsa>:thread: pcmState=%d\n", pcmState);
#endif // USE_ASYNC
            }
         } // if st->b_running
      }
...
void _PaStream::processInput(sBool _bVerbose) {
   ...
   if(avail > 0)
   {
      if(_bVerbose)
      {
         Dprintf("[trc] PaStream<alsa>::processInput: avail=%d\n", avail);
      }

      while(b_active && avail > 0)
      {
         sSI curNumFrames;
         ...
         if(ringAvail < maxRingAvail)
         {
            ...
            {
               Dprintf("[~~~] PaStream<alsa>::processInput: snd_pcm_readi() returned frames=%d but requested curNumFrames=%u\n", frames, curNumFrames);
            }
         ...
         // if(avail > 0)
         // {
         //    yac_host->printf("[~~~] PaStream<alsa>::processInput: input overflow (%d frames left), skipping..\n", avail);
         //    snd_pcm_sframes_t frames = snd_pcm_readi(in_handle,
         //                                             in_buf,
         //                                             avail
         //                                             );
...
void _PaStream::processOutput(sBool _bVerbose) {
   ...
   if(avail > 0)
   {
      if(_bVerbose)
      {
         Dprintf("[trc] PaStream<alsa>::processOutput: avail=%d\n", avail);
      }
...
//             }
         ...
         if(ringAvail > 0u)
         {
            if(_bVerbose)
            {
               Dprintf("[trc] PaStream<alsa>::processOutput: ringAvail=%u\n", ringAvail);
            }

            if( sSI(ringAvail) < curNumFrames )
            {
               // yac_host->printf("[~~~] PaStream<alsa>::processOutput: output underrun, ringAvail=%u, avail=%d\n", ringAvail, avail);
...
#undef Dclipv
            ...
            if(_bVerbose)
            {
               Dprintf("[trc] PaStream<alsa>::processOutput: processed curNumFrames=%d => new avail=%d\n", curNumFrames, avail);
            }
            ...
            if(_bVerbose)
            {
               Dprintf("[trc] PaStream<alsa>::processOutput: snd_pcm_writei() returned %d. remaining avail=%d\n", frames, avail);
            }

            if(frames != (snd_pcm_sframes_t)curNumFrames)
            {
               Dprintf("[~~~] PaStream<alsa>::processOutput: snd_pcm_writei() returned frames=%d but requested curNumFrames=%u\n", frames, curNumFrames);
            }
         ...
         // if(avail > 0)
         // {
         //    yac_host->printf("[~~~] PaStream<alsa>::processOutput: output underrun (need %d. filling with zero..)\n", avail);
...
sBool _PaStream::hwParamsSet(snd_pcm_t *handle, snd_pcm_hw_params_t *hwparams, int _line) {
   ...
   if(err < 0)
   {
      yac_host->printf("[---] PaStream<alsa>::hwParamsSet:%d: snd_pcm_hw_params() failed. err=%d(\"%s\")\n",
                       _line,
                       err,
                       snd_strerror(err)
                       );
                           ...
                           )
{
   ...
   {
      yac_host->printf("[---] PaStream<alsa>::initStream: invalid output device index=%d\n", _outParams->device_index);
      return YAC_FALSE;
   }
   ...
   {
      yac_host->printf("[---] PaStream<alsa>::initStream: output device index=%d is NULL\n", _outParams->device_index);
      return YAC_FALSE;
   }
   ...
   // Open device
   {
      ...
      Dprintf("xxx PaStream<alsa>::initStream2: _inParams->channel_count=%u\n", _inParams->channel_count);

      if(_inParams->channel_count > 0)
      {
         err = snd_pcm_open(&in_handle, (const char*)deviceInfoOut->hw_name->chars, SND_PCM_STREAM_CAPTURE, 0);
         Dprintf("xxx PaStream<alsa>::initStream2: snd_pcm_open in_handle err=%d\n", err);
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_open(dev=\"%s\") failed (playback): err=%d(\"%s\").\n",
      ...
      yac_host->printf("[dbg] PaStream<alsa>::initStream: prefPeriodSize=%lu, prefNumPeriods=%d, prefBufferSize=%lu.\n",
                       prefPeriodSize,
                       prefNumPeriods,
                       prefBufferSize
                       );
      ...
      if(err < 0)
      {
         yac_host->printf("[---] PaStream<alsa>::initStream<out>: snd_pcm_hw_params_any() failed. err=%d(\"%s\")\n",
                          err,
                          snd_strerror(err)
                          );
         closeHandles();
      ...
      // configuration
      {
         ...
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_set_access() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_set_format() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
         ...
         // Query min/max rates
         {
            ...
            if(err < 0)
            {
               yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_get_rate_min() failed. err=%d(\"%s\")\n",
                                err,
                                snd_strerror(err)
                                );
            }
            ...
            if(err < 0)
            {
               yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_get_rate_max() failed. err=%d(\"%s\")\n",
                                err,
                                snd_strerror(err)
                                );
            }

            yac_host->printf("[dbg] PaStream<alsa>::initStream: rateMin=%u rateMax=%u\n", rateMin, rateMax);
         }
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_set_rate() failed. rate=%u, err=%d(\"%s\")\n",
                             sample_rate,
                             err,
                             snd_strerror(err)
                             );
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_set_channels() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_get_buffer_size_min() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
         }
         ...
         if(err < 0)
         {
            yac_host->printf("[~~~] PaStream<alsa>::initStream: snd_pcm_hw_params_get_buffer_size_max() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
         }

         yac_host->printf("[dbg] PaStream<alsa>::initStream: bufSizeMin=%u bufSizeMax=%u\n",
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_set_buffer_size() failed. prefBufferSize=%u, err=%d(\"%s\")\n",
                             prefBufferSize,
                             err,
                             snd_strerror(err)
                             );
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_get_period_size_min() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
         }
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_get_period_size_max() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
         }
         yac_host->printf("[dbg] PaStream<alsa>::initStream: periodSizeMin=%lu periodSizeMax=%lu\n",
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_get_periods_min() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
         }
         ...
         {
            yac_host->printf("[dbg] PaStream<alsa>::initStream: snd_pcm_hw_params_get_periods_min() returned %u\n",
                             periods
                             );
         }
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_get_periods_max() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
         }
         ...
         {
            yac_host->printf("[dbg] PaStream<alsa>::initStream: snd_pcm_hw_params_get_periods_max() returned %u\n",
                             periods
                             );
         }
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_set_periods() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
         }
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_set_period_size() failed. prefPeriodSize=%lu err=%d(\"%s\")\n",
                             prefPeriodSize,
                             err,
                             snd_strerror(err)
                             );
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_get_period_size() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
         }
         ...
         {
            yac_host->printf("[dbg] PaStream<alsa>::initStream: snd_pcm_hw_param_get_period_size() returned %lu.\n",
                             periodSize
                             );
         }
         ...
         // Query actual #periods
         {
            ...
            if(err < 0)
            {
               yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_get_periods() failed. err=%d(\"%s\")\n",
                                err,
                                snd_strerror(err)
                                );
            }
            ...
            {
               yac_host->printf("[dbg] PaStream<alsa>::initStream: snd_pcm_hw_params_get_periods(): periods=%u\n", periods);
            }
         }
         ...
         if(err < 0) {
            yac_host->printf("[---] PaStream<alsa>::initStream: snd_pcm_hw_params_get_buffer_size() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
         ...
         {
            yac_host->printf("[dbg] PaStream<alsa>::initStream: snd_pcm_hw_params_get_buffer_size() returned bufferSize=%lu\n",
                             bufferSize
                             );
         }
         ...
         yac_host->printf("[dbg] PaStream<alsa>::initStream: #periods=%u\n", periods);
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>: snd_pcm_hw_params_get_rate() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
         ...
         {
            yac_host->printf("[dbg] PaStream<alsa>: snd_pcm_hw_params_get_rate(): sample_rate=%u.\n",
                             sample_rate
                             );
         }
      ...
      if(NULL != in_handle)
      {
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<in>: snd_pcm_hw_params_any() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
         ...
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<in>: snd_pcm_hw_params_set_access() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<in>: snd_pcm_hw_params_set_format() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<in>: snd_pcm_hw_params_set_rate() failed. rate=%u, err=%d(\"%s\")\n",
                             sample_rate,
                             err,
                             snd_strerror(err)
                             );
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<in>: snd_pcm_hw_params_set_channels() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<in>: snd_pcm_hw_params_set_buffer_size() failed. prefBufferSize=%u, err=%d(\"%s\")\n",
                             prefBufferSize,
                             err,
                             snd_strerror(err)
                             );
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<in>: snd_pcm_hw_params_set_periods() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
         }
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<in>: snd_pcm_hw_params_set_period_size() failed. prefPeriodSize=%lu err=%d(\"%s\")\n",
                             prefPeriodSize,
                             err,
                             snd_strerror(err)
                             );
      ...
      if(NULL == out_handle)
      {
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<in>: snd_async_add_pcm_handler() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
      ...
      if(NULL != out_handle)
      {
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<out>: snd_async_add_pcm_handler() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
      ...
      yac_host->printf("[dbg] PaStream<alsa>::initStream: bufSize=%u\n",
                       (sUI)bufferSize
                       );

      yac_host->printf("[dbg] PaStream<alsa>::initStream: bufSize=%u, periodSize=%u, #periods=%u.\n",
                       (sUI)bufferSize,
                       (sUI)periodSize,
                       (sUI)periods
                       );
      ...
      if((0u == num_periods) && (0u != period_size))
      {
         ...
         yac_host->printf("[~~~] PaStream<alsa>::initStream: num_periods==0, forcing num_periods=%u/%u=%u\n",
      ...
      else if(num_periods <= 1u)
      {
         ...
         yac_host->printf("[~~~] PaStream<alsa>::initStream: num_periods<=1, forcing num_periods=2\n");
      }

      if(NULL != in_handle)
      {
         err = snd_pcm_prepare(in_handle);
         Dprintf("[trc] PaStream<alsa>::initStream<in>: snd_pcm_prepare() returned %d\n", err);
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<in>: snd_pcm_prepare() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
      ...
      if(NULL != out_handle)
      {
         ...
         Dprintf("[trc] PaStream<alsa>::initStream<out>: snd_pcm_prepare() returned %d\n", err);
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::initStream<out>: snd_pcm_prepare() failed. err=%d(\"%s\")\n",
                             err,
                             snd_strerror(err)
                             );
            closeHandles();
...
//           );
   ...
   //   (note) don't. may lead to deadlock issue when streamcallback depends on worker thread that is
   //           synced to same mutex as "initStream" caller
   if(0)
   {
   ...
   if(NULL != in_handle)
   {
      ...
      {
         yac_host->printf("[dbg] PaStream<alsa>::initStream: input thread started.\n");
      }
      else
      {
         yac_host->printf("[---] PaStream<alsa>::initStream: input pthread_create() failed.\n");
         closeHandles();
         return YAC_FALSE;
      }
   }
   ...
   if(NULL != out_handle)
   {
      ...
      {
         yac_host->printf("[dbg] PaStream<alsa>::initStream: output thread started.\n");
      }
      else
      {
         yac_host->printf("[---] PaStream<alsa>::initStream: output pthread_create() failed.\n");
         closeHandles();
         return YAC_FALSE;
      }
   }
   ...
   // Create worker thread
   {
      ...
      {
         yac_host->printf("[dbg] PaStream<alsa>::initStream: thread started.\n");
         return YAC_TRUE;
      }
      else
      {
         yac_host->printf("[---] PaStream<alsa>::initStream: pthread_create() failed.\n");
         closeHandles();
         return YAC_FALSE;
      }
   }
                           ...
                           )
{
   ...
   yac_host->printf("[trc] PaStream<alsa>::initStream: suggested latency is %u frames @%uHz => #per=%u #perSz=%u\n", bufSz, _sampleRate, numPeriods, periodSz);
...
void _PaStream::_close(void) {
   if(b_running)
   {
      if(NULL != out_handle)
      {
         if(b_out_running)
         {
            // Wait until output thread has exited
            *(volatile sBool*)&b_out_running = YAC_FALSE;
            pthread_join(pthread_id_out, NULL);
            yac_host->printf("[dbg] PaStream<alsa>::close: output thread terminated.\n");
         }
         // waitProcessCbk();

         if(b_in_running)
         {
            // Wait until input thread has exited
            *(volatile sBool*)&b_in_running = YAC_FALSE;
            pthread_join(pthread_id_in, NULL);
            yac_host->printf("[dbg] PaStream<alsa>::close: input thread terminated.\n");
         }

         // Wait until worker thread has exited
         *(volatile sBool*)&b_running = YAC_FALSE;
         pthread_join(pthread_id, NULL);
         yac_host->printf("[dbg] PaStream<alsa>::close: worker thread terminated.\n");
...
void _PaStream::callStreamCallback(const sUI _numFrames) {
   ...
   if(numSamples <= ip->max_elements)
   {
      ...
      if(numSamples <= op->max_elements)
      {
         ...
         yac_host->yacContextSetDefault(stream_cbk_context);
...
void _PaStream::_start(void) {
   if(NULL != out_handle)
   {
      ...
      Dprintf("[trc] PaStream<alsa>::start: stream start queued.\n");
      b_active = YAC_TRUE;
   }
}
...
void _PaStream::_stop(void) {
   if(NULL != out_handle)
   {
      if(b_active)
      {
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::stop: snd_pcm_drop() failed. err=%d(\"%s\")\n", err, snd_strerror(err));
         }
         ...
         if(err < 0)
         {
            yac_host->printf("[---] PaStream<alsa>::stop: snd_pcm_pause() failed. err=%d(\"%s\")\n", err, snd_strerror(err));
         }
         else
         {
            // ok
            yac_host->printf("[...] PaStream<alsa>::stop: stream stopped.\n");
         }

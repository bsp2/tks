...
YC class RecordedMIDIEvent : public YAC_Object {
   public:
      ...
      union {
         ...
         sUI msg;      // b_long==false, e.g. 0x007b48b1
         sU8 bytes[4];
      } data;
   ...
   public:
      ...
      */
      ...
      // see parseBuffer() (LocalMIDI mode)
...
public:
   MIDIHDR midi_buffers[MIDIIN_NUMBUFFERS];
   ...
   sUI   allocBufferIndex     (void);
   sBool receiveBufferIndex   (sSI _idx);
   sU8*  getBufferData        (sSI _idx);
   sUI   getBufferSize        (sSI _idx);
   sBool prepareNextBuffer    (void);
...
public:
   ...
   */
   ...
   YM sSI         _prepareSysEx  (void); // deprecated, use record ringbuf instead!
   YM sBool       _receiveSysEx  (sSI _idx, YAC_Object *_rcvBuf); // deprecated, use record ringbuf instead!
